...
def _obj_rule(model):
    return (model.I_OBJ_WT*sum(model.eps_I[i]**2 for i in model.S_FE) + \
        model.PHI_OBJ_WT*sum(model.P_DATA_WTS[i]*model.eps_phi[i]**2 for i in model.S_TRI))
...
def _beta_bar(model):
    return (model.beta_bar, sum(model.beta[i] for i in model.S_BETA)/len(model.S_BETA))
...
def _phidot_eq(model,i):
    ...
    fe = model.TIME.get_upper_element_boundary(i)
    j = model.TIME._fe.index(fe)
    return model.phidot[i] == model.eps_I[j] \
