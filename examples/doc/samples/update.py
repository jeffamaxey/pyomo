...
for Dir in glob.glob('case_studies/*'):
    ...
    if os.path.exists(fname):
        print >>OUTPUT, " * [wiki:Documentation/PyomoGallery/CaseStudies/%s %s]" % (dir, get_title(fname))
...
for Dir in glob.glob('scripts/*'):
    ...
    if os.path.exists(fname):
        print >>OUTPUT, " * [wiki:Documentation/PyomoGallery/Scripts/%s %s]" % (dir, get_title(fname))
...
for Dir in glob.glob('comparisons/*'):
    ...
    if os.path.exists(fname):
        print >>OUTPUT, " * [wiki:Documentation/PyomoGallery/ModelingComparisons/%s %s]" % (dir, get_title(fname))
